...
async function uploadFile(file: File) {
  ...
  await uploadImage(file)
    .catch(() => toast.add({ title: 'An error occured', description: 'Please try again', color: 'error' }))
...
<template>
  <div>
    ...
    >
      <UModal v-model:open="open" title="Login to upload images">
        <template #body>
          <LoginForm
      ...
      >
        ...
        >
          ...
          >
            ...
            <NuxtLink
              :to="`/examples/image/gallery/detail/${image.pathname.split('.')[0]}`"
