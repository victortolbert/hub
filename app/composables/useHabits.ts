...
function resetIfStreakBroken(habit: Habit): void {
  if (habit.complete_days.length === 0)
    return

  const sortedDays = habit.complete_days
    .slice()
    .sort((a, b) => compareAsc(parseISO(a), parseISO(b)))
  const hasGap = sortedDays.some(
    (day, index) =>
      index > 0
      && differenceInDays(parseISO(day), parseISO(sortedDays[index - 1])) > 1,
  )
...
function checkAllHabitsForStreak(): void {
  return habits.value.forEach(resetIfStreakBroken)
}

function addHabit(title: string, description: string): void {
...
function toggleTodayCompletion(habit: Habit): void {
  const isCompletedToday = habit.complete_days.some(day =>
    isSameDay(parseISO(day), parseISO(today)),
  )

  if (isCompletedToday) {
    habit.complete_days = habit.complete_days.filter(
      day => !isSameDay(parseISO(day), parseISO(today)),
    )
    ...
    if (
      habit.complete_days.length === habit.target_days
      && habit.target_days === 40
    ) {
